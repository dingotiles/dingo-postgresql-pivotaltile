...
bucket = s3.directories.get('dingo-postgresql-pivotaltile')
tiles = bucket.files.all('prefix' => 'dingo-postgresql-')
versions = tiles.inject([]) do |vs, tile|
  if tile.key =~ /^dingo-postgresql-(.*)\.pivotal$/
    vs << $1
  end
  vs
end
