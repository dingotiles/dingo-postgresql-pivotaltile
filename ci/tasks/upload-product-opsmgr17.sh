...
response=$(curl -f ${insecure} -H "Authorization: Bearer ${access_token}" \
  "${opsmgr_url}/api/v0/installations" -d "ignore_warnings=1&enabled_errands[${product_install_uuid}][post_deploy_errands][]=sanity-test&enabled_errands[${product_install_uuid}][post_deploy_errands][]=broker-registrar" -X POST)
